...
RUNNER_PATH = REPOSITORY_ROOT / "experiments/runners/high_level_multidomain_v1_r1.py"
MANIFEST_PATH = (
    REPOSITORY_ROOT / "experiments/manifests/high-level-multidomain-v1-r1.json"
)
...
STATE_ROOT = Path("/opt/angler/state/project-angler/high-level-multidomain-v1-r1")
SEALED_SEED_PATH = STATE_ROOT / "sealed/seeds-v1-r1.json"
QUALIFICATION_RELEASE_CLAIM_PATH = (
    STATE_ROOT / "sealed/qualification-release-claim-v1-r1.json"
)
EVALUATION_ADMISSION_CLAIM_PATH = (
    STATE_ROOT / "sealed/evaluation-admission-claim-v1-r1.json"
)
_ACTIVE_EVALUATION_PERMITS: dict[str, object] = {}
SCRATCH_ROOT = Path("/opt/angler/scratch/high-level-multidomain-v1-r1")
...
EVALUATION_RESULT_PATH = Path(
    "/opt/angler/results/high-level-multidomain-v1-r1.json"
)
QUALIFICATION_RUNTIME_ROOT = STATE_ROOT / "qualification-v1-r1"
...
def _source_inventory_paths() -> tuple[str, ...]:
    ...
    if (
        ...
        raise RunnerInvariantError("R1 construction source inventory differs")
    ...
    if set(frozen).intersection(REQUIRED_CONSTRUCTION_SOURCES):
        raise RunnerInvariantError("frozen and R1 construction sources overlap")
    combined = frozen + REQUIRED_CONSTRUCTION_SOURCES
    if len(combined) != 32 or len(set(combined)) != 32:
...
def _validate_fresh_r1_seed_commitments(
    ...
    values = tuple(_digest(value, "R1 seed commitment") for value in commitments)
    if len(values) != 2 or len(set(values)) != 2:
        raise RunnerInvariantError("R1 seed commitments must be exactly two and distinct")
    if set(values) & CONSUMED_V1_SEED_COMMITMENTS:
        raise RunnerInvariantError("R1 seed commitments overlap consumed V1")
