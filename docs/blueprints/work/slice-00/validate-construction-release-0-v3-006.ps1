...
$baselinePath = Join-Path $releaseRoot 'baselines\ANG-BASELINE-CR0-CONTINUITY-002.json'
$gatePath = Join-Path $releaseRoot 'gates\ANG-GATE-CR0-CONTINUITY-002.md'
$leafPath = Join-Path $script:RepoRoot 'docs\blueprints\work\slice-00\ANG-WORK-CR0-CONTINUITY-002.md'
$specPath = Join-Path $releaseRoot 'revalidations\ANG-CR0-REVALIDATION-20260825-006.md'
$decisionPath = Join-Path $releaseRoot 'revalidations\ANG-CR0-REVALIDATION-20260825-006-decision.json'
$receiptPath = Join-Path $releaseRoot 'branch-receipts\CONTINUITY.md'
$selfPath = $MyInvocation.MyCommand.Path

$expectedBaseCommit = '84ff08b484197755c3fed66e7dc06988539e456e'
$expectedBaselineHash = '0BEF86FC8A56870E4B94BE1E057FD3C975D97D66DB1C4B775CC273AB373FFCE9'
$expectedGateHash = '947A78F2E7EA9528B5B1997A0DA178E125BF0B0E05C7192B5547E31BFF7A2919'
$expectedLeafHash = '67873CE6B8CF51702357ED7D8473E023FEAEF1B9459313E1955719BB1F681680'
...
Assert-FileHash -RelativePath 'docs/blueprints/releases/construction-0/baselines/ANG-BASELINE-CR0-CONTINUITY-002.json' -ExpectedHash $expectedBaselineHash
Assert-FileHash -RelativePath 'docs/blueprints/releases/construction-0/gates/ANG-GATE-CR0-CONTINUITY-002.md' -ExpectedHash $expectedGateHash
Assert-FileHash -RelativePath 'docs/blueprints/work/slice-00/ANG-WORK-CR0-CONTINUITY-002.md' -ExpectedHash $expectedLeafHash
...
$expectedProjectionTargets = [ordered]@{
    ...
    'docs/blueprints/work/slice-00/handoffs/ANG-WORK-CR0-CONTINUITY-002.md' = @{ state = 'absent'; sha256 = $null; class = 'preserve_on_failure' }
...
try {
    ...
    if ($baseline.baseline_id -ne 'ANG-BASELINE-CR0-CONTINUITY-002' -or $baseline.base_commit -ne $expectedBaseCommit -or [int]$baseline.leaf_revision -ne 1) {
...
if ($null -eq $gateFront) {
    ...
    Assert-ScalarFields -FrontMatter $gateFront -Label 'Continuity gate' -Expected @{
        ...
        independent_reviewer_instance = 'ANG-REVIEW-CODEX-SAFETY-CR0-CONTINUITY-002'
...
if ($null -eq $leafFront) {
    ...
    Assert-ScalarFields -FrontMatter $leafFront -Label 'Continuity leaf' -Expected @{
        ...
        rollback_ref = "ANG-BASELINE-CR0-CONTINUITY-002@sha256:$expectedBaselineHash"
        human_flourishing_gate = 'ANG-GATE-HUMAN-FLOURISHING-001@1-NOT_RUN'
        normal_evidence_gate = 'ANG-GATE-EVIDENCE-SCHEMAS-001@1-NOT_RUN'
        normal_resources_gate = 'ANG-GATE-RESOURCE-DESIGN-001@1-NOT_RUN'
    }
...
$roleInstances = @(
    'ANG-EXEC-CODEX-ROOT-CR0-CONTINUITY-002',
    'ANG-AUTH-VALIDATOR-001',
    'ANG-REVIEW-CODEX-SAFETY-CR0-CONTINUITY-002',
    'ANG-REVIEW-CODEX-SAFETY-CR0-REVALIDATION-006',
    'ANG-BP-ROOT'
)
...
if (-not (Test-Path -LiteralPath $specPath -PathType Leaf)) {
    Add-ValidationError 'Revalidation-006 specification is missing'
}
...
if ($null -eq $specFront) {
    Add-ValidationError 'Revalidation-006 front matter is missing'
}
else {
    Assert-ScalarFields -FrontMatter $specFront -Label 'Revalidation-006' -Expected @{
        revalidation_id = 'ANG-CR0-REVALIDATION-20260825-006'
        status = 'PENDING'
        release_manifest_version = '3'
...
$pendingPhaseLine = '> **PENDING / NON-AUTHORIZING.** Revalidation 006 has no decision; `ANG-WORK-CR0-CONTINUITY-002@1` has no execution authority.'
$authorizedPhaseLine = '> **AUTHORIZED ONLY FOR `ANG-WORK-CR0-CONTINUITY-002@1`.** Revalidation 006 is independently `APPROVED`; authority exists only after this validator returns AUTHORIZED PASS.'
...
function Assert-DecisionObject {
    ...
    if ($Decision.revalidation_id -ne 'ANG-CR0-REVALIDATION-20260825-006' -or $Decision.disposition -ne 'APPROVED') {
    ...
    if ($Decision.reviewer_role -ne 'ANG-AUTH-SAFETY-APPROVER-001' -or
        $Decision.reviewer_instance -ne 'ANG-REVIEW-CODEX-SAFETY-CR0-REVALIDATION-006' -or
...
function Assert-AuthorizedStaticState {
    ...
    Assert-DecisionObject -Decision $Decision -Label "$Label revalidation-006 decision"
...
if ($phase -eq 'PENDING') {
    if (Test-Path -LiteralPath $decisionPath) {
        Add-ValidationError 'PENDING phase requires the revalidation-006 decision path to be absent'
    }
...
elseif ($phase -eq 'AUTHORIZED') {
    if (-not (Test-Path -LiteralPath $decisionPath -PathType Leaf)) {
        Add-ValidationError 'AUTHORIZED phase requires the revalidation-006 decision file'
    }
    else {
        $actualDecisionHash = Get-Sha256 -Path $decisionPath
        if ($decisionHashField -ne $actualDecisionHash) {
            Add-ValidationError 'AUTHORIZED manifest does not pin the exact revalidation-006 decision hash'
        }
        try {
            ...
            Add-ValidationError "Unable to validate revalidation-006 decision: $($_.Exception.Message)"
        }
    }
}
...
Require-LiteralText -Raw $manifestRaw -Values @(
    ...
    'The active 006 role partition contains exactly sixteen executor outputs and one reviewer receipt',
...
) -Label 'Revalidation-006 procedure'
...
if ($phase -eq 'PENDING') {
    Write-Output 'Construction Release 0 v3-006 validation PASS - PENDING / NON-AUTHORIZING; AUTHORIZED_TRANSITION_SELF_TEST PASS; 13 exact projections preserved, 4 future outputs absent, and no leaf execution authority exists.'
}
else {
    Write-Output 'Construction Release 0 v3-006 validation PASS - revalidation 006 independently APPROVED and Manifest v3 authorized only for ANG-WORK-CR0-CONTINUITY-002@1; all 17 targets remain at pre-start baseline.'
}
