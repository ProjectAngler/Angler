...
$baselinePath = Join-Path $releaseRoot 'baselines\ANG-BASELINE-CR0-CONTINUITY-003.json'
$gatePath = Join-Path $releaseRoot 'gates\ANG-GATE-CR0-CONTINUITY-003.md'
$leafPath = Join-Path $script:RepoRoot 'docs\blueprints\work\slice-00\ANG-WORK-CR0-CONTINUITY-003.md'
$specPath = Join-Path $releaseRoot 'revalidations\ANG-CR0-REVALIDATION-20260825-007.md'
$decisionPath = Join-Path $releaseRoot 'revalidations\ANG-CR0-REVALIDATION-20260825-007-decision.json'
$receiptPath = Join-Path $releaseRoot 'branch-receipts\CONTINUITY-003.md'
...
$expectedBaseCommit = '8a62f9508c1e8f573a57a32fd99e880aefea7ae6'
$expectedBaselineHash = '134A6EDF9D608C5CDED1812C9CB4A0F63AEA99ACBCF814A79CF00EDF83492D28'
$expectedGateHash = 'C7FF7AD57661D7EF2303D628E6A69A0F54C3366FA163143BA7E35EBAE76479F9'
$expectedLeafHash = 'D4D15CC6C725416BB5FDCB8DC6833EB913B3A4EFD8B34303672D57D6DC072BCD'
...
Assert-FileHash -RelativePath 'docs/blueprints/releases/construction-0/baselines/ANG-BASELINE-CR0-CONTINUITY-003.json' -ExpectedHash $expectedBaselineHash
Assert-FileHash -RelativePath 'docs/blueprints/releases/construction-0/gates/ANG-GATE-CR0-CONTINUITY-003.md' -ExpectedHash $expectedGateHash
Assert-FileHash -RelativePath 'docs/blueprints/work/slice-00/ANG-WORK-CR0-CONTINUITY-003.md' -ExpectedHash $expectedLeafHash
...
$expectedProjectionTargets = [ordered]@{
    ...
    'docs/blueprints/branches/evidence/children/evidence-schemas/handoffs/2026-08-25-cr0-scaffold-accepted-corrective-003.md' = @{ state = 'absent'; sha256 = $null; class = 'preserve_on_failure' }
    'tests/synthetic/slice00/continuity/Test-Cr0ContinuationConsistency-003.ps1' = @{ state = 'absent'; sha256 = $null; class = 'restore_or_remove' }
    'docs/blueprints/work/slice-00/handoffs/ANG-WORK-CR0-CONTINUITY-003.md' = @{ state = 'absent'; sha256 = $null; class = 'preserve_on_failure' }
    'docs/blueprints/releases/construction-0/branch-receipts/CONTINUITY-003.md' = @{ state = 'absent'; sha256 = $null; class = 'preserve_on_failure' }
}

try {
    $baseline = Get-Content -Raw -LiteralPath $baselinePath | ConvertFrom-Json
    if ($baseline.baseline_id -ne 'ANG-BASELINE-CR0-CONTINUITY-003' -or $baseline.base_commit -ne $expectedBaseCommit -or [int]$baseline.leaf_revision -ne 1) {
...
if ($null -eq $gateFront) {
    ...
    Assert-ScalarFields -FrontMatter $gateFront -Label 'Continuity gate' -Expected @{
        gate_id = 'ANG-GATE-CR0-CONTINUITY-003'
        version = '1'
        status = 'specified'
        activation_state = 'unusable_pending_revalidation'
        activation_revalidation = 'ANG-CR0-REVALIDATION-20260825-007'
        release_manifest_version = '3'
        leaf = 'ANG-WORK-CR0-CONTINUITY-003@1'
        baseline_sha256 = $expectedBaselineHash
        executor = 'ANG-EXEC-CODEX-ROOT-CR0-CONTINUITY-003'
        executor_task_id = '01a03a80-bb20-7d01-acf6-f50ca4856be5'
        independent_verifier = 'ANG-AUTH-VALIDATOR-001'
        independent_reviewer_instance = 'ANG-REVIEW-CODEX-SAFETY-CR0-CONTINUITY-003'
        ...
        decision_path = 'docs/blueprints/releases/construction-0/branch-receipts/CONTINUITY-003.md'
...
$reviewerPath = 'docs/blueprints/releases/construction-0/branch-receipts/CONTINUITY-003.md'
$expectedDeniedPaths = @(
...
if ($null -eq $leafFront) {
    ...
    Assert-ScalarFields -FrontMatter $leafFront -Label 'Continuity leaf' -Expected @{
        blueprint_id = 'ANG-WORK-CR0-CONTINUITY-003'
        ...
        execution_owner = 'ANG-EXEC-CODEX-ROOT-CR0-CONTINUITY-003'
        executor_task_id = '01a03a80-bb20-7d01-acf6-f50ca4856be5'
        authorized_write_scope_owner = 'ANG-EXEC-CODEX-ROOT-CR0-CONTINUITY-003'
        independent_validator = 'ANG-AUTH-VALIDATOR-001'
        independent_gate_reviewer_instance = 'ANG-REVIEW-CODEX-SAFETY-CR0-CONTINUITY-003'
        ...
        independent_gate_write_scope_instance = 'ANG-REVIEW-CODEX-SAFETY-CR0-CONTINUITY-003'
        reviewer_vocabulary_ack = 'ACK_ACCEPTED'
        gate_sha256 = $expectedGateHash
        rollback_ref = "ANG-BASELINE-CR0-CONTINUITY-003@sha256:$expectedBaselineHash"
        human_flourishing_gate = 'ANG-GATE-HUMAN-FLOURISHING-001@1-NOT_RUN'
        normal_evidence_gate = 'ANG-GATE-EVIDENCE-SCHEMAS-001@1-NOT_RUN'
        normal_resources_gate = 'ANG-GATE-RESOURCE-DESIGN-001@1-NOT_RUN'
    }
...
Require-LiteralText -Raw $leafRaw -Values @(
    '$scriptPath="tests/synthetic/slice00/continuity/Test-Cr0ContinuationConsistency-003.ps1"',
...
$roleInstances = @(
    'ANG-EXEC-CODEX-ROOT-CR0-CONTINUITY-003',
    'ANG-AUTH-VALIDATOR-001',
    'ANG-REVIEW-CODEX-SAFETY-CR0-CONTINUITY-003',
    'ANG-REVIEW-CODEX-SAFETY-CR0-REVALIDATION-007',
    'ANG-BP-ROOT'
)
...
if (-not (Test-Path -LiteralPath $specPath -PathType Leaf)) {
    Add-ValidationError 'Revalidation-007 specification is missing'
}
...
if ($null -eq $specFront) {
    Add-ValidationError 'Revalidation-007 front matter is missing'
}
else {
    Assert-ScalarFields -FrontMatter $specFront -Label 'Revalidation-007' -Expected @{
        revalidation_id = 'ANG-CR0-REVALIDATION-20260825-007'
        status = 'PENDING'
        release_manifest_version = '3'
...
$pendingPhaseLine = '> **PENDING / NON-AUTHORIZING.** Revalidation 007 has no decision; `ANG-WORK-CR0-CONTINUITY-003@1` has no execution authority.'
$authorizedPhaseLine = '> **AUTHORIZED ONLY FOR `ANG-WORK-CR0-CONTINUITY-003@1`.** Revalidation 007 is independently `APPROVED`; authority exists only after this validator returns AUTHORIZED PASS.'
...
function Assert-DecisionObject {
    ...
    if ($Decision.revalidation_id -ne 'ANG-CR0-REVALIDATION-20260825-007' -or $Decision.disposition -ne 'APPROVED') {
    ...
    if ($Decision.reviewer_role -ne 'ANG-AUTH-SAFETY-APPROVER-001' -or
        $Decision.reviewer_instance -ne 'ANG-REVIEW-CODEX-SAFETY-CR0-REVALIDATION-007' -or
...
function Assert-AuthorizedStaticState {
    ...
    Assert-DecisionObject -Decision $Decision -Label "$Label revalidation-007 decision"
...
if ($phase -eq 'PENDING') {
    if (Test-Path -LiteralPath $decisionPath) {
        Add-ValidationError 'PENDING phase requires the revalidation-007 decision path to be absent'
    }
...
elseif ($phase -eq 'AUTHORIZED') {
    if (-not (Test-Path -LiteralPath $decisionPath -PathType Leaf)) {
        Add-ValidationError 'AUTHORIZED phase requires the revalidation-007 decision file'
    }
    else {
        $actualDecisionHash = Get-Sha256 -Path $decisionPath
        if ($decisionHashField -ne $actualDecisionHash) {
            Add-ValidationError 'AUTHORIZED manifest does not pin the exact revalidation-007 decision hash'
        }
        try {
            ...
            Add-ValidationError "Unable to validate revalidation-007 decision: $($_.Exception.Message)"
        }
    }
}
...
Require-LiteralText -Raw $manifestRaw -Values @(
    ...
    'The active 007 role partition contains exactly sixteen executor outputs and one fresh reviewer receipt',
...
) -Label 'Revalidation-007 procedure'
...
if ($phase -eq 'PENDING') {
    Write-Output 'Construction Release 0 v3-007 validation PASS - PENDING / NON-AUTHORIZING; AUTHORIZED_TRANSITION_SELF_TEST PASS; 13 exact projections preserved, 4 future outputs absent, and no leaf execution authority exists.'
}
else {
    Write-Output 'Construction Release 0 v3-007 validation PASS - revalidation 007 independently APPROVED and Manifest v3 authorized only for ANG-WORK-CR0-CONTINUITY-003@1; all 17 targets remain at pre-start baseline.'
}
