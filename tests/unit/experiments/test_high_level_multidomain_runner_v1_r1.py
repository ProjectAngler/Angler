...
from experiments.runners import high_level_multidomain_v1_r1 as runner  # noqa: E402
...
class CanonicalAndIntentTests(unittest.TestCase):
    ...
    def test_r1_run_identities_are_distinct_from_frozen_evaluator_identities(
        self,
    ) -> None:
        self.assertEqual(
        ...
        self.assertEqual(
            ...
            "angler.high-level-multidomain.v1-r1-harness-qualification",
        )
        self.assertEqual(
            runner.EVALUATION_IDENTITY,
            "angler.high-level-multidomain.v1-r1-evaluation",
        )
        self.assertNotEqual(
            runner.QUALIFICATION_IDENTITY,
            runner.EVALUATOR_QUALIFICATION_IDENTITY,
...
class FrozenSourceTests(unittest.TestCase):
    def test_every_frozen_predecessor_hash_matches_the_active_leaf(self) -> None:
        self.assertEqual(len(_FROZEN_SOURCE_HASHES), 29)
    ...
    def test_r1_runner_is_an_exact_normalized_mechanical_delta(self) -> None:
        legacy = (ROOT / "experiments/runners/high_level_multidomain_v1.py").read_text(
            encoding="utf-8"
        )
        ...
        source_block = re.compile(
            r'    \(\n'
            r'        "docs/blueprints/branches/science/work/"\n'
            r'        "ANG-WORK-SCIENCE-HIGH-LEVEL-MULTIDOMAIN-ADAPTATION-V1-001\.md"\n'
            r'    \): "b8678803ac2ea63f44d9f1e386dffefb907be619d208c18aba71de31e39e629a",\n'
            r'    "experiments/manifests/high-level-multidomain-v1\.json": \(\n'
            r'        "91542cc7557495c7793f82247772e71b0f00f6995e3b4b3fbf2185ee38b470f8"\n'
            r'    \),\n'
            r'    "experiments/runners/high_level_multidomain_v1\.py": \(\n'
            r'        "4c9dc1551d5902f4132ac8d7262a025c17d72299de79d872f0b300d0299356d7"\n'
            r'    \),\n'
            r'    "tests/integration/runtime/test_high_level_multidomain_qwen_v1\.py": \(\n'
            r'        "05836ee77994f8905a8a2523135872a81b8b67bcdae77e102e730ac9ef55025d"\n'
            r'    \),\n'
            r'    "tests/unit/experiments/test_high_level_multidomain_runner_v1\.py": \(\n'
            r'        "5a225fdd57b8690cf33b63eed5d0d8cf25d575ee46d929a7e9e67f7329d9ae92"\n'
            r'    \),\n'
        )
        recovery, source_substitutions = source_block.subn("", recovery)
        self.assertEqual(source_substitutions, 1)
        ...
        def replace_once(current: str, predecessor: str) -> None:
            nonlocal recovery
            self.assertEqual(recovery.count(current), 1)
            recovery = recovery.replace(current, predecessor, 1)

        # The successor leaf authorizes only the missing live-evaluation
        # ownership/CLI boundary.  Normalize that additive integrity plumbing
        # before retaining the predecessor's whole-file semantic comparison.
        replace_once("import errno\n", "")
        replace_once(
            '    "ANG-WORK-SCIENCE-HIGH-LEVEL-MULTIDOMAIN-ADAPTATION-V1-R1-"\n'
            '    "LIVE-EVALUATION-COMPLETION-001.md"',
            '    "ANG-WORK-SCIENCE-HIGH-LEVEL-MULTIDOMAIN-ADAPTATION-V1-R1-001.md"',
        )
        remove_block(
            r"\nEVALUATION_RUNTIME_ROOT = STATE_ROOT / \"evaluation-v1-r1\".*?"
            r"EVALUATION_COGNEE_SCOPES_ROOT = STATE_ROOT / \"evaluation-cognee-scopes\"\n"
        )
        for row in (
...
''',
            '            "scope_state_parent": str(STATE_ROOT / "cognee-scopes"),\n',
        )
        remove_block(
        ...
        remove_block(
            ...
            r"        qualification_release, _ = _load_qualification_release_claim\(.*?"
            r"            \)\n(?=    sources = manifest\[\"frozen_sources\"\])"
        )
        replace_once(
            "async def _run_and_publish_evaluation_once(\n",
            "async def run_and_publish_evaluation_once(\n",
        )
        replace_once(
            '    """Private claim-to-terminal core used by the zero-injection live CLI."""\n',
            '    """One live claim -> release -> terminal publication path, with no resume."""\n',
        )
        remove_block(
        ...
        remove_block(
            ...
            r"(?=\n\nasync def _run_live_qualification_cli\()"
        )
        remove_block(
            r"\nasync def _run_live_evaluation_cli\(.*?"
...
''',
            ...
            self.worker_tmpdir,
            self.worker_cwd,
            self.worker_tmpdir.parent,
            self.worker_cwd.parent,
        ):
            try:
                path.rmdir()
            except FileNotFoundError:
...
''',
            '''    # Final evaluation remains fail-closed until its separately audited live
    # dependency owner and one-use terminal path are complete.
    raise RunnerInvariantError(
        f"{intent.mode.value} live execution is not yet admitted by this runner build"
    )
''',
        )
        ...
        remove_block(
            r"\ndef _validate_fresh_r1_seed_commitments\(.*?"
            r"(?=\n\ndef qualification_replicate_commitment\()"
        )
        remove_block(
            r"\ndef _validate_evaluator_handshake_identity\(.*?"
            r"(?=\n\ndef _validated_evaluator_timeout\()"
        )
        identity_block = (
            ...
            'QUALIFICATION_IDENTITY = f"{SUITE_SCHEMA}-r1-harness-qualification"\n'
            'EVALUATION_IDENTITY = f"{SUITE_SCHEMA}-r1-evaluation"'
        )
        legacy_identity_block = (
            'QUALIFICATION_IDENTITY = f"{SUITE_SCHEMA}-harness-qualification"\n'
            'EVALUATION_IDENTITY = f"{SUITE_SCHEMA}-evaluation"'
        )
        self.assertEqual(recovery.count(identity_block), 1)
        recovery = recovery.replace(identity_block, legacy_identity_block)
        self.assertEqual(recovery.count("EVALUATOR_EVALUATION_IDENTITY"), 3)
        self.assertEqual(recovery.count("EVALUATOR_QUALIFICATION_IDENTITY"), 1)
        recovery = recovery.replace(
            "EVALUATOR_EVALUATION_IDENTITY",
            "EVALUATION_IDENTITY",
        ).replace(
...
""",
        ...
        recovery = recovery.replace(
            "    _validate_fresh_r1_seed_commitments(commitments)\n",
            "",
        )
        self.assertNotIn("_validate_fresh_r1_seed_commitments", recovery)
        replace_once(
...
""",
            """    sources = dict(sorted(source_hashes.items()))
    required = set(FROZEN_SOURCE_SHA256) | set(REQUIRED_CONSTRUCTION_SOURCES)
""",
        )
        replace_once(
...
""",
            """    for name, digest in sources.items():
        _text(name, "source path", 512)
        if Path(name).is_absolute() or ".." in Path(name).parts:
            raise ValueError("source paths must be repository-relative and normalized")
        _raw_digest(digest, "source sha256")
""",
        )
        replace_once(
...
""",
            """    required = set(FROZEN_SOURCE_SHA256) | set(REQUIRED_CONSTRUCTION_SOURCES)
    observed = {
        relative: sha256_file(root / relative)[0]
        for relative in sorted(required)
    }
""",
        )
        ...
        raise RunnerInvariantError("accepted R1 science leaf identity differs")
""",
            "    _raw_digest(accepted_leaf_sha256, \"accepted leaf sha256\")\n",
            1,
        )
...
""",
        ...
        recovery = recovery.replace(
            ...
            "if random_diagnostic and (not feedback or not transition):",
        )
        recovery = recovery.replace(
            "sealed / SEALED_SEED_PATH.name,\n"
            "                *((sealed / QUALIFICATION_RELEASE_CLAIM_PATH.name,) if release_required else ()),",
            'sealed / "seeds-v1.json",\n'
            '                *((sealed / "qualification-release-claim-v1.json",) if release_required else ()),',
        )
        recovery = recovery.replace(
            "high_level_multidomain_v1_r1",
            "high_level_multidomain_v1",
        )
        recovery = recovery.replace(
            "high_level_multidomain_runner_v1_r1",
            "high_level_multidomain_runner_v1",
        )
        recovery = recovery.replace(
            "high_level_multidomain_qwen_v1_r1",
            "high_level_multidomain_qwen_v1",
        )
        recovery = recovery.replace(
            "high-level-multidomain-v1-r1",
            "high-level-multidomain-v1",
        )
        recovery = recovery.replace(
            "ANG-WORK-SCIENCE-HIGH-LEVEL-MULTIDOMAIN-ADAPTATION-V1-R1-001",
            "ANG-WORK-SCIENCE-HIGH-LEVEL-MULTIDOMAIN-ADAPTATION-V1-001",
        )
        recovery = recovery.replace("seeds-v1-r1.json", "seeds-v1.json")
        recovery = recovery.replace(
            "qualification-release-claim-v1-r1.json",
            "qualification-release-claim-v1.json",
        )
        recovery = recovery.replace(
            "evaluation-admission-claim-v1-r1.json",
            "evaluation-admission-claim-v1.json",
        )
        recovery = recovery.replace("qualification-v1-r1", "qualification-v1")
        while "\n\n\n" in recovery:
            recovery = recovery.replace("\n\n\n", "\n\n")
        while "\n\n\n" in legacy:
            legacy = legacy.replace("\n\n\n", "\n\n")
        self.assertEqual(recovery, legacy)

    def test_r1_paths_are_exact_and_disjoint_from_every_v1_write_identity(
        self,
    ) -> None:
        self.assertEqual(
            runner.RUNNER_PATH,
        ...
        self.assertEqual(
            runner.STATE_ROOT,
            Path("/opt/angler/state/project-angler/high-level-multidomain-v1-r1"),
        )
        self.assertEqual(
            runner.SCRATCH_ROOT,
            Path("/opt/angler/scratch/high-level-multidomain-v1-r1"),
        )
        self.assertEqual(runner.SEALED_SEED_PATH.name, "seeds-v1-r1.json")
        self.assertEqual(
        ...
        self.assertEqual(runner.QUALIFICATION_RUNTIME_ROOT.name, "qualification-v1-r1")
        self.assertEqual(runner.EVALUATION_RUNTIME_ROOT.name, "evaluation-v1-r1")
        self.assertEqual(
        ...
        recovery_mutable_paths = (
            runner.RUNNER_PATH,
            ROOT / "tests/unit/experiments/test_high_level_multidomain_runner_v1_r1.py",
            ROOT / "tests/integration/runtime/test_high_level_multidomain_qwen_v1_r1.py",
            runner.MANIFEST_PATH,
        ...
        for recovery_path in recovery_mutable_paths:
            for legacy_path in legacy_mutable_paths:
                with self.subTest(recovery=recovery_path, legacy=legacy_path):
                    self.assertNotEqual(recovery_path, legacy_path)
                    self.assertNotIn(recovery_path, legacy_path.parents)
                    self.assertNotIn(legacy_path, recovery_path.parents)
    ...
    def test_all_immutable_v1_witnesses_and_source_inventories_are_exact(self) -> None:
        self.assertEqual(
        ...
        self.assertEqual(len(runner.FROZEN_SOURCE_SHA256), 29)
        self.assertEqual(
            ...
            completed = runner.subprocess.run(
                (
                    ...
                    (
                        ...
                        "high_level_multidomain_v1_r1; print('IMPORT_OK')"
                    ),
                ),
                cwd=temporary,
